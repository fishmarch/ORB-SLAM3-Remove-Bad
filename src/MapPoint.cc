...
MapPoint::MapPoint():
        ...
        mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0), mnIndexForCompression(0),
        mnCorrectedReference(0), mnBAGlobalForKF(0), mnVisible(1), mnFound(1), mbBad(false), mfChi2InLocalBA(0.0),
        mpReplaced(shared_ptr<MapPoint>()), mnUpdateChi2ForKF(0), mnChi2Times(0), mnOptimizedTimesInLBA(0)
{
}
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, shared_ptr<KeyFrame> pRefKF, Map* pMap):
        ...
        mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(pRefKF), mnVisible(1), mnFound(1), mbBad(false), mfChi2InLocalBA(0.0),
        mpReplaced(shared_ptr<MapPoint>()), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap), mnIndexForCompression(0),
        mnOriginMapId(pMap->GetId()), mnChi2Times(0), mnOptimizedTimesInLBA(0)
{
    SetWorldPos(Pos);
...
MapPoint::MapPoint(const double invDepth, cv::Point2f uv_init, shared_ptr<KeyFrame> pRefKF, shared_ptr<KeyFrame> pHostKF, Map* pMap):
        ...
        mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(pRefKF), mnVisible(1), mnFound(1), mbBad(false), mfChi2InLocalBA(0.0),
        mpReplaced(shared_ptr<MapPoint>()), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap), mnIndexForCompression(0),
        mnOriginMapId(pMap->GetId()), mnUpdateChi2ForKF(0), mnChi2Times(0), mnOptimizedTimesInLBA(0)
{
    mInvDepth=invDepth;
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
        mnFirstKFid(-1), mnFirstFrame(pFrame->mnId), nObs(0), mnTrackReferenceForFrame(0), mnLastFrameSeen(0), mbOptimizedInLocalBA(false),
        mnBALocalForKF(0), mnFuseCandidateForKF(0), mnLoopPointForKF(0), mnCorrectedByKF(0), mnIndexForCompression(0),
        mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(shared_ptr<KeyFrame>()), mnVisible(1), mfChi2InLocalBA(0.0),
        mnFound(1), mbBad(false), mpReplaced(shared_ptr<MapPoint>()), mpMap(pMap), mnOriginMapId(pMap->GetId()), mnUpdateChi2ForKF(0), mnChi2Times(0),
