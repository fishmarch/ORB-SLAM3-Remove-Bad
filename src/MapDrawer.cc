...
namespace ORB_SLAM3 {
    ...
    void MapDrawer::DrawMapPoints() {
        ...
        const vector<shared_ptr<MapPoint>> &vpRefMPs = pActiveMap->GetReferenceMapPoints();
        ...
        for (set<shared_ptr<MapPoint>>::iterator sit = spRefMPs.begin(), send = spRefMPs.end(); sit != send; sit++) {
            if (!(*sit) || (*sit)->isBad())
                continue;
    ...
    void MapDrawer::DrawKeyFrames(const bool bDrawKF, const bool bDrawGraph, const bool bDrawInertialGraph,
        ...
        const vector<shared_ptr<KeyFrame> > vpKFs = pActiveMap->GetAllKeyFrames();
        ...
        if (bDrawGraph) {
            ...
            for (size_t i = 0; i < vpKFs.size(); i++) {
                if (!vpKFs[i] || vpKFs[i]->isBad())
                    continue;
                // Covisibility Graph
                const vector<shared_ptr<KeyFrame> > vCovKFs = vpKFs[i]->GetCovisiblesByWeight(100);
                Eigen::Vector3f Ow = vpKFs[i]->GetCameraCenter();
                if (!vCovKFs.empty()) {
                    for (vector<shared_ptr<KeyFrame> >::const_iterator vit = vCovKFs.begin(), vend = vCovKFs.end();
                         vit != vend; vit++) {
                ...
                // Spanning tree
                shared_ptr<KeyFrame> pParent = vpKFs[i]->GetParent();
                if (pParent) {
                ...
                // Loops
                set<shared_ptr<KeyFrame> > sLoopKFs = vpKFs[i]->GetLoopEdges();
                for (set<shared_ptr<KeyFrame> >::iterator sit = sLoopKFs.begin(), send = sLoopKFs.end(); sit != send; sit++) {
        ...
        if (bDrawInertialGraph && pActiveMap->isImuInitialized()) {
            ...
            for (size_t i = 0; i < vpKFs.size(); i++) {
                ...
                shared_ptr<KeyFrame> pNext = pKFi->mNextKF;
                if (pNext) {
