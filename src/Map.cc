...
namespace ORB_SLAM3 {
    ...
    //To see whether bad mappoints and keyframes will be deleted
    void Map::DeleteBadMapPoints() {
        unique_lock<mutex> lock(mMutexMap);
        int num = 0;
        for (auto i = mspMapPointsToDelete.begin(), iend = mspMapPointsToDelete.end(); i != iend ;) {
            if((*i).use_count() < 2) {
                num++;
                i = mspMapPointsToDelete.erase(i);
            }else {
                i++;
            }
        }
        int numKF = 0;
        for (auto i = mspKeyFramesToDelete.begin(), iend = mspKeyFramesToDelete.end(); i != iend ;) {
            if((*i).use_count() < 2) {
                numKF++;
                i = mspKeyFramesToDelete.erase(i);
            }else {
                i++;
            }
        }
        cout << "MP: " << num << "/" << mspMapPointsToDelete.size() + num << endl;
        cout << "KF: " << numKF << "/" << mspKeyFramesToDelete.size() + numKF << endl;
    }

    void Map::EraseMapPoint(shared_ptr<MapPoint> pMP) {
        unique_lock<mutex> lock(mMutexMap);
        mspMapPoints.erase(pMP);
//        mspMapPointsToDelete.insert(pMP);
...
//        mspKeyFramesToDelete.insert(pKF);
