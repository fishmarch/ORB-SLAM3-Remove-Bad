...
KeyFrame::KeyFrame():
        ...
        mfGridElementWidthInv(0), mfGridElementHeightInv(0), mnTrackReferenceForFrame(0), mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnBALocalForMerge(0),
        ...
        NLeft(0),NRight(0), mnNumberOfOpt(0), mbHasVelocity(false), mbCompressed(false)
{
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB, Tracking* pTracking):
    ...
    fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy), mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn),
    mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors), mpTracking(pTracking),
    ...
    mpCamera(F.mpCamera), mpCamera2(F.mpCamera2), mbCompressed(false),
