...
namespace ORB_SLAM3 {
    ...
    void Optimizer::BundleAdjustment(const vector<shared_ptr<KeyFrame> > &vpKFs, const vector<shared_ptr<MapPoint>> &vpMP,
        ...
        for (size_t i = 0; i < vpMP.size(); i++) {
            ...
            int nEdges = 0;
            //SET EDGES
            for (map<shared_ptr<KeyFrame> , tuple<int, int>>::const_iterator mit = observations.begin();
    ...
    void Optimizer::FullInertialBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId,
        ...
        for (size_t i = 0; i < vpMPs.size(); i++) {
            ...
            //Set edges
            for (map<shared_ptr<KeyFrame> , tuple<int, int>>::const_iterator mit = observations.begin(), mend = observations.end();
                 mit != mend; mit++) {
    ...
    void Optimizer::LocalBundleAdjustment(shared_ptr<KeyFrame> pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF,
        ...
        for (list<shared_ptr<KeyFrame> >::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
             lit != lend; lit++) {
        ...
        for (list<shared_ptr<MapPoint>>::iterator lit = lLocalMapPoints.begin(), lend = lLocalMapPoints.end();
             lit != lend; lit++) {
            map<shared_ptr<KeyFrame> , tuple<int, int>> observations = (*lit)->GetObservations();
            for (map<shared_ptr<KeyFrame> , tuple<int, int>>::iterator mit = observations.begin(), mend = observations.end();
                 mit != mend; mit++) {
                shared_ptr<KeyFrame> pKFi = mit->first;
        ...
        optimizer.initializeOptimization();
        int iterationTimes = optimizer.optimize(10);
        ...
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
            ...
            e->computeError();
            float fChi2 = e->chi2();
            if (fChi2 > 5.991 || !e->isDepthPositive()) {
                e->setLevel(1);
        ...
        for (size_t i = 0, iend = vpEdgesStereo.size(); i < iend; i++) {
            ...
            e->computeError();
            float fChi2 = e->chi2();
            if (fChi2 > 7.815 || !e->isDepthPositive()) {
                e->setLevel(1);
    ...
    void Optimizer::OptimizeEssentialGraph(Map *pMap, shared_ptr<KeyFrame>pLoopKF, shared_ptr<KeyFrame>pCurKF,
        ...
        // Set Loop edges
        int count_loop = 0;
        for (map<shared_ptr<KeyFrame>, set<shared_ptr<KeyFrame>> >::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.end();
             mit != mend; mit++) {
            shared_ptr<KeyFrame>pKF = mit->first;
            ...
            for (set<shared_ptr<KeyFrame>>::const_iterator sit = spConnections.begin(), send = spConnections.end();
                 sit != send; sit++) {
        ...
        for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
            ...
            for (set<shared_ptr<KeyFrame>>::const_iterator sit = sLoopEdges.begin(), send = sLoopEdges.end();
                 sit != send; sit++) {
    ...
    void Optimizer::OptimizeEssentialGraph(shared_ptr<KeyFrame>pCurKF, vector<shared_ptr<KeyFrame>> &vpFixedKFs,
    ...
    int Optimizer::OptimizeSim3(shared_ptr<KeyFrame>pKF1, shared_ptr<KeyFrame>pKF2, vector<shared_ptr<MapPoint>> &vpMatches1, g2o::Sim3 &g2oS12,
                                const float th2,
    ...
    void Optimizer::LocalInertialBA(shared_ptr<KeyFrame>pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF, int &num_OptKF,
        ...
        // Set Fixed KeyFrame vertices
        for (list<shared_ptr<KeyFrame>>::iterator lit = lFixedKeyFrames.begin(), lend = lFixedKeyFrames.end();
             lit != lend; lit++) {
            shared_ptr<KeyFrame>pKFi = *lit;
            VertexPose *VP = new VertexPose(pKFi);
            VP->setId(pKFi->mnId);
...
//        }
    ...
    void
    Optimizer::LocalBundleAdjustment(shared_ptr<KeyFrame>pMainKF, vector<shared_ptr<KeyFrame>> vpAdjustKF, vector<shared_ptr<KeyFrame>> vpFixedKF,
                                     bool *pbStopFlag) {
    ...
    void Optimizer::MergeInertialBA(shared_ptr<KeyFrame>pCurrKF, shared_ptr<KeyFrame>pMergeKF, bool *pbStopFlag, Map *pMap,
